...
const deleteAssistant = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: assistantsController.deleteAssistant - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await assistants_1.default.deleteAssistant(req.params.id, req.query.isDeleteBoth, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const getAllAssistants = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: assistantsController.getAllAssistants - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await assistants_1.default.getAllAssistants(workspaceId, type);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const getAssistantById = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: assistantsController.getAssistantById - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await assistants_1.default.getAssistantById(req.params.id, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const updateAssistant = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: assistantsController.updateAssistant - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await assistants_1.default.updateAssistant(req.params.id, req.body, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const getDocumentStores = async (req, res, next) => {
    try {
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: assistantsController.getDocumentStores - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await assistants_1.default.getDocumentStores(workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
