...
const getChatflowById = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: chatflowsController.getChatflowById - workspace ${workspaceId} not found!`);
        }
