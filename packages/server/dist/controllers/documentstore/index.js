...
const getAllDocumentStores = async (req, res, next) => {
    try {
        ...
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.getAllDocumentStores - workspaceId not provided!`);
        }
        const apiResponse = await documentstore_1.default.getAllDocumentStores(workspaceId, page, limit);
...
const getDocumentStoreById = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.getDocumentStoreById - workspaceId not provided!`);
        }
        const apiResponse = await documentstore_1.default.getDocumentStoreById(req.params.id, workspaceId);
        if (apiResponse && apiResponse.whereUsed) {
            apiResponse.whereUsed = JSON.stringify(await documentstore_1.default.getUsedChatflowNames(apiResponse, workspaceId));
        }
        return res.json(Interface_1.DocumentStoreDTO.fromEntity(apiResponse));
    }
    catch (error) {
...
const getDocumentStoreFileChunks = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.getDocumentStoreFileChunks - workspaceId not provided!`);
        }
        ...
        const apiResponse = await documentstore_1.default.getDocumentStoreFileChunks(appDataSource, req.params.storeId, req.params.fileId, workspaceId, page);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const deleteDocumentStoreFileChunk = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.deleteDocumentStoreFileChunk - workspaceId not provided!`);
        }
        const apiResponse = await documentstore_1.default.deleteDocumentStoreFileChunk(req.params.storeId, req.params.loaderId, req.params.chunkId, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const editDocumentStoreFileChunk = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.editDocumentStoreFileChunk - workspaceId not provided!`);
        }
        const apiResponse = await documentstore_1.default.editDocumentStoreFileChunk(req.params.storeId, req.params.loaderId, req.params.chunkId, body.pageContent, body.metadata, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const saveProcessingLoader = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.saveProcessingLoader - workspaceId not provided!`);
        }
        const apiResponse = await documentstore_1.default.saveProcessingLoader(appServer.AppDataSource, body, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const updateDocumentStore = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.updateDocumentStore - workspaceId not provided!`);
        }
        const store = await documentstore_1.default.getDocumentStoreById(req.params.id, workspaceId);
        if (!store) {
...
const deleteVectorStoreFromStore = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.deleteVectorStoreFromStore - workspaceId not provided!`);
        }
        const apiResponse = await documentstore_1.default.deleteVectorStoreFromStore(req.params.storeId, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const saveVectorStoreConfig = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.saveVectorStoreConfig - workspaceId not provided!`);
        }
        const apiResponse = await documentstore_1.default.saveVectorStoreConfig(appDataSource, body, true, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const updateVectorStoreConfigOnly = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.PRECONDITION_FAILED, `Error: documentStoreController.updateVectorStoreConfigOnly - workspaceId not provided!`);
        }
        const apiResponse = await documentstore_1.default.updateVectorStoreConfigOnly(body, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
