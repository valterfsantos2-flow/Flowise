...
const createEvaluation = async (req, res, next) => {
    try {
        ...
        const apiResponse = await evaluations_1.default.createEvaluation(body, baseURL, orgId, workspaceId);
...
const runAgain = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluationsService.runAgain - workspace ${workspaceId} not found!`);
        }
        const httpProtocol = req.get('x-forwarded-proto') || req.get('X-Forwarded-Proto') || req.protocol;
        const baseURL = `${httpProtocol}://${req.get('host')}`;
        const apiResponse = await evaluations_1.default.runAgain(req.params.id, baseURL, orgId, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const getEvaluation = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluationsService.getEvaluation - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluations_1.default.getEvaluation(req.params.id, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const deleteEvaluation = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluationsService.deleteEvaluation - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluations_1.default.deleteEvaluation(req.params.id, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const getAllEvaluations = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluationsService.getAllEvaluations - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluations_1.default.getAllEvaluations(workspaceId, page, limit);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const isOutdated = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluationsService.isOutdated - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluations_1.default.isOutdated(req.params.id, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const getVersions = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluationsService.getVersions - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluations_1.default.getVersions(req.params.id, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const patchDeleteEvaluations = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluationsService.patchDeleteEvaluations - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluations_1.default.patchDeleteEvaluations(ids, workspaceId, isDeleteAllVersion);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
