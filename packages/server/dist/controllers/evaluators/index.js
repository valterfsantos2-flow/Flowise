...
const getAllEvaluators = async (req, res, next) => {
    try {
        ...
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluatorService.getAllEvaluators - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluator_1.default.getAllEvaluators(workspaceId, page, limit);
...
const getEvaluator = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluatorService.getEvaluator - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluator_1.default.getEvaluator(req.params.id, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const updateEvaluator = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluatorService.updateEvaluator - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluator_1.default.updateEvaluator(req.params.id, req.body, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
...
const deleteEvaluator = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: evaluatorService.deleteEvaluator - workspace ${workspaceId} not found!`);
        }
        const apiResponse = await evaluator_1.default.deleteEvaluator(req.params.id, workspaceId);
        return res.json(apiResponse);
    }
    catch (error) {
        next(error);
