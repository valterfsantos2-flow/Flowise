...
const deleteVariable = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: variablesController.deleteVariable - workspace ${workspaceId} not found!`);
        }
...
const getAllVariables = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: variablesController.getAllVariables - workspace ${workspaceId} not found!`);
        }
...
const updateVariable = async (req, res, next) => {
    try {
        ...
        const workspaceId = req.user?.activeWorkspaceId;
        if (!workspaceId) {
            throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.NOT_FOUND, `Error: variablesController.updateVariable - workspace ${workspaceId} not found!`);
        }
