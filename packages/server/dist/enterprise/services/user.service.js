    ...
    async updateUser(newUserData) {
        ...
        try {
            ...
            if (newUserData.oldPassword && newUserData.newPassword && newUserData.confirmPassword) {
                ...
                // verify old password
                if (!(0, encryption_util_1.compareHash)(newUserData.oldPassword, oldUserData.credential)) {
                    throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.BAD_REQUEST, "Invalid User Credential" /* UserErrorMessage.INVALID_USER_CREDENTIAL */);
                }
                if (newUserData.newPassword !== newUserData.confirmPassword) {
                    throw new internalFlowiseError_1.InternalFlowiseError(http_status_codes_1.StatusCodes.BAD_REQUEST, "Passwords do not match" /* UserErrorMessage.PASSWORDS_DO_NOT_MATCH */);
                }
                const hash = (0, encryption_util_1.getHash)(newUserData.newPassword);
            ...
            if (newUserData.oldPassword && newUserData.newPassword && newUserData.confirmPassword) {
                await (0, SessionPersistance_1.destroyAllSessionsForUser)(updatedUser.id);
            }
        }
        catch (error) {
